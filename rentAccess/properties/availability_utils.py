...
def get_booked_days_from_dates(date_start, date_end):
    d_range = pd.date_range(date_start, date_end, freq='d')
    return d_range
...
def get_number_of_weeks_from_dates(d1, d2):
    monday1 = (d1 - timedelta(days=d1.weekday()))
    monday2 = (d2 - timedelta(days=d2.weekday()))
    return int((monday2 - monday1).days / 7) + 1


def decouple_weeks_into_dates(date_start, date_end):
    for i in range(get_number_of_weeks_from_dates(date_start, date_end)):
        for j in range(pend.date(date_start.year,
                                 date_start.month,
                                 date_start.day).end_of('week').day - date_start.day):
            print(date(date_start.year, date_start.month,
                                 date_start.day) + timedelta(days=j, weeks=i-1))


def confirm_open_days(available_days, date_start, date_end):
    days_booked = get_booked_days_from_dates(date_start, date_end)
    days = available_days_to_db(available_days)
    for day_b in days_booked:
        if days[day_b.day_of_week] == '0':
            return False
    return True
